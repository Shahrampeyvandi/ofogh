...
class AppCustomerController extends Controller
{
    public function index()
    {
        ...
        foreach ($appmenus as $keym => $appmenu) {
            ...
            if ($appmenu->type == 'محصول انتخابی') {
                foreach ($array as $keyn => $arr) {
                    $product = Product::where('id', $arr)->first();
                    $ser['iditem'] = $product->id;
                    $ser['title'] = $product->product_name;
                    $ser['icon'] = $product->product_picture;
                    $array[$keyn] = $ser;
                }
                $appmenus[$keym]->item = $array;
            } else if ($appmenu->type == 'محصول های دسته') {
                foreach ($array as $keyn => $arr) {
                    $products = Product::where('category_id', $arr)->get();
                    foreach ($products as $key => $product) {
                        $cat['iditem'] = $product->id;
                        $cat['title'] = $product->product_name;
                        $cat['icon'] = $product->product_picture;
                        $array[$key] = $cat;
                    }
                    $appmenus[$keym]->iditem = $arr;
                }
                $categoryname = Category::where('id', $arr)->first();
                $catpar = Category::where('id', $categoryname->parent_id)->first();
                do {
                    $appmenus[$keym]->titlecat = $categoryname->name;
                    $categoryname = Category::where('id', $categoryname->parent_id)->first();
                } while ($categoryname);

                $appmenus[$keym]->item = $array;
            } else if ($appmenu->type == 'لیست دسته بندی') {

                foreach ($array as $keyn => $arr) {
                    $service = Category::where('id', $arr)->first();
                    $ser['iditem'] = $service->id;
                    $ser['title'] = $service->name;
                    $ser['icon'] = $service->picture;
                    $array[$keyn] = $ser;
                }
